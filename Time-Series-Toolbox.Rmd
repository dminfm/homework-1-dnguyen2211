...
```
Your answer: Trotz Transformation wurden ein ähnliches Ergebnis im Vergleich zum Original erzeugt.
...
Your answer: 
Das residual ist ein white noise, was auch erwartet wird.
Das Residual ist Normalverteilt.
...
Your answer: False, das Model könnte das falsche für das Problem sein
...
```{r}
# todo
 checkresiduals(myts.test)
```

Your answer: Nicht normalverteilt 
...
```{r}
# todo
 donaive1 <- naive(dowjones, h=5)
donaive2 <- naive(dowjones, h=10) 
donaive3 <- naive(dowjones, h=15) 

domeanf1 <- meanf(dowjones, h=5)
domeanf2 <- meanf(dowjones, h=10) 
domeanf3 <- meanf(dowjones, h=15) 

autoplot(dowjones) +
  autolayer(donaive3, PI=FALSE, series="Naive 15") +
  autolayer(donaive2, PI=FALSE, series="Naive 10") +
  autolayer(donaive1, PI=FALSE, series="Naive 5") +
  xlab("Time") + ylab("Closing Price (US$)") +
  ggtitle("Dow Jones index") +
  guides(colour=guide_legend(title="Forecast"))

autoplot(dowjones) +
  autolayer(domeanf3, PI=FALSE, series="meanf 15") +
  autolayer(domeanf2, PI=FALSE, series="meanf 10") +
  autolayer(domeanf1, PI=FALSE, series="meanf 5") +
  xlab("Time") + ylab("Closing Price (US$)") +
  ggtitle("Dow Jones index") +
  guides(colour=guide_legend(title="Forecast"))
```
...
# todo
# todo
ibm.test <- window(ibmclose, start = 300)
ibm.train <- window(ibmclose, end = c(300, 69))
...
```{r}
# todo
 hsales.test <- window(hsales, start = c(1994, 2))
hsales.train <- window(hsales, end = 1994)
...
hsalesm.f <- meanf(hsales.train, h=h)
hsalesrw.f <- naive(hsales.train, h=h)
hsalesrwd.f <- rwf(hsales.train, drift=TRUE, h=h)
hsalesrwds.f <- snaive(hsales.train, h=h)

autoplot(hsales) +
  xlab("Day") +
  ggtitle("hsales") +
  autolayer(hsalesm.f$mean, col=2, series="Mean method") +
  autolayer(hsalesrw.f$mean, col=3, series="Naive method") +
  autolayer(hsalesrwd.f$mean, col=4, series="Drift method") + 
  autolayer(hsalesrwds.f$mean, col=5, series="SNaive method")
